...
const DEFAULT_SCRIPT = "很多老板买了十几个AI工具，最后还是没人真正用起来。问题不在工具，而在业务流程没有被重新设计。真正有价值的AI，不是多一个聊天框，而是把选题、内容、获客、跟进和复盘连成一条自动运转的生产线。先选一个最贵、最慢、最依赖老板经验的流程，把它做成智能体，企业才会真正看到结果。";
...
Page({
  ...
  async cloneVoice() {
    ...
    await this.runAction("clone", async () => {
      ...
      await this.loadVoices(job.result?.voice_id || job.result?.id);
